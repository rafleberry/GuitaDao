...
/**
 *  This contract allows Guita seed investors and advisors to claim tokens.
...
contract InvestorClaimV2 is Ownable {
    ...
    function _claim(uint256 _amount) internal returns (uint256 toSend_) {
        ...
        terms[msg.sender].gClaimed = info.gClaimed.add(gGUITA.balanceTo(toSend_));
    }
    ...
    function claimed(address _address) public view returns (uint256) {
        return gGUITA.balanceFrom(terms[_address].gClaimed);
    }
    ...
    function migrate(address[] memory _addresses) external onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            ...
            setTerms(_addresses[i], term.max, term.percent, term.wClaimed);
        }
    }

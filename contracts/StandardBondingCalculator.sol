...
contract GuitaBondingCalculator is IBondingCalculator {
    ...
    function markdown(address _pair) external view override returns (uint256) {
        ...
        uint256 reserve;
        if (IUniswapV2Pair(_pair).token0() == address(GUITA)) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair(_pair).token1() == address(GUITA), "Invalid pair");
            reserve = reserve0;
        }
        return reserve.mul(2 * (10**IERC20Metadata(address(GUITA)).decimals())).div(getTotalValue(_pair));
    }
}

...
contract sGuita is IsGUITA, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgGUITA public gGUITA; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked GUITA", "sGUITA", 9) ERC20Permit("Staked GUITA") {
    ...
    function setgGUITA(address _gGUITA) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gGUITA) == address(0), "gGUITA:  gGUITA already set");
        require(_gGUITA != address(0), "gGUITA:  gGUITA is not a valid contract");
    ...
    /**
        @notice increases rGUITA supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sGUITA of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sGUITA
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sGUITA: insufficient balance");
    }
    ...
    // toG converts an sGUITA balance to gGUITA terms. gGUITA is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gGUITA.balanceTo(amount);
    }

    // fromG converts a gGUITA balance to sGUITA terms. sGUITA is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gGUITA.balanceFrom(amount);
    }
    ...
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply
                ...
                .add(gGUITA.balanceFrom(IERC20(address(gGUITA)).totalSupply()))

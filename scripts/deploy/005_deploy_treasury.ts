...
import { GuitaTreasury__factory } from "../../types";
...
const func: DeployFunction = async (hre: HardhatRuntimeEnvironment) => {
    ...
    const treasuryDeployment = await deploy(CONTRACTS.treasury, {
        from: deployer,
        args: [guitaDeployment.address, TREASURY_TIMELOCK, authorityDeployment.address],
        log: true,
        skipIfAlreadyDeployed: false,
    });

    await GuitaTreasury__factory.connect(treasuryDeployment.address, signer);
};
